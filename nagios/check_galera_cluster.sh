...
while getopts “hvu:p:H:P:w:s:a:c:m:f:0” OPTION; do
  case $OPTION in
    h)
      print_help
      exit $ST_UK
...
mysqlhost=`rancher --access-key=${RANCHER_ACCESS_KEY} --secret-key=${RANCHER_SECRET_KEY} --url https://rancher.bookbooncloud.com ps -c | grep -E ${ranchermatch} | grep ${rancherhostid}| awk '{print $6}'`
